...
import { Eye, EyeOff, Loader2, Mail, Zap } from "lucide-react"
import { saveQuickLoginSession, hasQuickLoginSessions } from "@/lib/quick-login-utils"
import { QuickLoginPanel } from "./quick-login-panel"
...
const FormLogin = ({
  ...
  // Verificar si hay inicio rápido disponible al montar
  useEffect(() => {
    setShowQuickLogin(hasQuickLoginSessions());
  }, []);
  ...
  async function onSubmit(values: z.infer<typeof loginSchema>) {
    startTransition(async () => {
      ...
      if (response.error) {
        ...
        // Manejar la opción "Recordarme" para inicio de sesión rápido
        if (values.rememberMe && response.userData) {
          saveQuickLoginSession(
            response.userData.email, 
            response.userData.name || response.userData.email,
          ...
          <CardContent className="space-y-4">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                {/* Opción de recordar para inicio de sesión rápido */}
                <FormField
                  control={form.control}
                  name="rememberMe"
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-start space-x-3 space-y-0 mb-4 ml-2">
                      <FormControl>
                        <Checkbox
                          checked={field.value}
                      ...
                      <div className="space-y-1 leading-none">
                        <FormLabel className="text-sm font-normal text-slate-600 cursor-pointer">
                          Recordarme para inicio rápido
                        </FormLabel>
                        <p className="text-xs text-slate-500">
                          Aparecerá un botón de inicio rápido en futuros logins
                        </p>
                      </div>
                    </FormItem>
                  )}
            ...
            {showQuickLogin ? (
              <div className="grid grid-cols-2 gap-3">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className="w-full h-11 bg-gradient-to-r from-yellow-50 to-orange-50 border-yellow-300 hover:from-yellow-100 hover:to-orange-100 hover:border-yellow-400 transition-all"
                    >
                      <Zap className="h-4 w-4 mr-2 text-yellow-600" />
                      <span className="text-slate-700 font-medium">Inicio Rápido</span>
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent 
              ...
              <div className="flex justify-center">
                <ButtonSocial provider="google" className="w-full max-w-xs">
                  <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path
                      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                      fill="#4285F4"
